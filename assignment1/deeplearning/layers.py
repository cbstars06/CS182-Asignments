...
def affine_forward(x, w, b):
    ...
    D = np.prod(arr)
    x_new = x.reshape((x.shape[0],D))
    b_new = b.reshape((1,b.shape[0]))
    out = x_new.dot(w) + b_new
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    cache = (x, w, b)
...
def batchnorm_backward(dout, cache):
  ...
  dgamma = x_norm.T.dot(dout)
  dgamma = np.diag(dgamma)
  dbeta = dout
  ...
  b = var + eps
  m = x.shape[0]
  dvar = np.sum(dx_norm * a, axis=0) * ((-1/2) * (b ** (-3/2)))
  dmean = np.sum(dx_norm * (-1/np.sqrt(b)), axis=0) + dvar * np.mean(-2*a)
  dx = dx_norm * (b) ** (-1/2) + dvar * 2 * a /m + dmean/m
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
      ...
      cache = [x, sample_mean, sample_var, eps, x_norm, gamma, beta]
        ...
        out = out*gamma.reshape((1,C,1,1)) + beta.reshape((1,C,1,1))
